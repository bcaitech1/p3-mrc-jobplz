...
class SparseRetrieval:
    ...
    # 어떻게 retrieve를 할 것인지
    def retrieve_with_option(self, query_or_dataset, topk=100,opt=None):

        if not opt:
            self.retrieve(query_or_dataset,topk)
        elif opt=='el':
            self.retrieve_with_elastic(query_or_dataset,topk=100)

        return None

    def retrieve(self, query_or_dataset, topk=25):
