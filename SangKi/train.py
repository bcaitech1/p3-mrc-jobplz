...
from retrieve_kss import SparseRetrieval
...
def main():
    ...
    if other_args.use_wandb:
        wandb.login()
        wandb.init(project=other_args.project_name)
        wandb.run.name = training_args.run_name
        wandb.save()
        # training_args.run_name = f"{model_args.model_name_or_path}_learning_rate={training_args.learning_rate}"
    ...
    if other_args.retrieval=='sp': # sparse embedding 방식
    ...
    elif other_args.retrieval=='de': # dense embedding 방식
        # run_dense_embedding()
        # if training_args.do_train or training_args.do_eval:
        #     run_mrc(data_args, training_args, model_args, datasets, tokenizer, model)
        return None

    elif other_args.retrieval=='1':
        return None

    elif other_args.retrieval=='2':
        return None
...
def run_mrc_de(data_args, training_args, model_args, datasets, tokenizer, model):
    run_mrc()
