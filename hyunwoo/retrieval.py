...
class SparseRetrieval:
    def __init__(self, tokenize_fn, data_path="./data/", context_path="wikipedia_documents.json"):
        ...
        # sentence embedding by Pororo
        self.se = Pororo(task="sentence_embedding", lang='ko')
        self.sts = Pororo(task="similarity", lang="ko")
        # should run get_sparse_embedding() or build_faiss() first.
        self.p_embedding = None
        self.indexer = None

    
    def get_pororo_embedding(self):
        # Pickle save
        pickle_name = f'PORORO_embedding.bin'
        ...
        if os.path.isfile(emd_path):
            ...
            for context in self.contexts:
                self.p_embedding.append(self.se(context))
            self.p_embedding = np.array(self.p_embedding)
    ...
    def pororo_retrieve(self, query_or_dataset, topk=1):
        assert self.p_embedding is not None, "You must build faiss by self.get_sparse_embedding() before you run self.retrieve()."
        if isinstance(query_or_dataset, str):
            doc_scores, doc_indices = self.get_relevant_doc_pororo(query_or_dataset, k=topk)
            print("[Search query]\n", query_or_dataset, "\n")
            for i in range(topk):
            ...
            with timer("query exhaustive search"):
                doc_scores, doc_indices = self.get_relevant_doc_bulk_pororo(query_or_dataset['question'], k=topk)
    ...
    def get_relevant_doc_pororo(self, query,  k=1):
        """
        참고: vocab 에 없는 이상한 단어로 query 하는 경우 assertion 발생 (예) 뙣뙇?
        """
        with timer("transform"):
            query_vec = self.se(query)
        ...
        with timer("query ex search"):
            ...
            l2_norm = np.sqrt(np.sum(np.square(query_vec))) * \
                        np.sqrt(np.sum(np.square(self.p_embedding), axis=1))
    ...
    def get_relevant_doc_bulk_pororo(self, queries, k=1):
        query_vec = []
        for query in tqdm(queries):
            query_vec.append(self.se(query))
        ...
        result = np.dot(query_vec , self.p_embedding.T)
        query_vec_norm = np.sqrt(np.sum(np.square(query_vec), axis=1))
        p_embedding_norm = np.sqrt(np.sum(np.square(self.p_embedding), axis=1))
        l2_norm = np.dot(query_vec_norm.reshape(query_vec_norm.shape[0], 1), 
                        p_embedding_norm.reshape(1,p_embedding_norm.shape[0]))
        result = result / l2_norm
        if not isinstance(result, np.ndarray):
            result = result.toarray()
        doc_scores = []
            ...
            for idx, example in enumerate(tqdm(query_or_dataset, desc="Sparse retrieval: ")):
                ...
                for i, doc_id in enumerate(relev_doc_ids):
                    tmp = {
                        ...
                        "context_id": doc_id,  # retrieved id
                        "context": self.contexts[doc_id],  # retrieved doument
                        "score" : self.sts(self.contexts[doc_id][:512], example['question'])                       
                                    # TF-IDF score : doc_scores[idx][i]
                    }
                    if 'context' in example.keys() and 'answers' in example.keys():
                        tmp["original_context"] = example['context']  # original document
                        tmp["answers"] = example['answers']           # original answer
