...
def load_data(config, tokenizer) :
    ...
    if os.path.exists(config.data_file) :
        with open(config.data_file, "rb") as handle :
            contexts, train_qa, valid_qa, _ = pickle.load(handle)
        return contexts, train_qa, valid_qa
    context_hash = set()
    contexts = []
    train_qa = parse_squad_data(config, "train", tokenizer, context_hash, contexts)
    valid_qa = parse_squad_data(config, "validation", tokenizer, context_hash, contexts)
    parse_wiki_data(config, tokenizer, context_hash, contexts)

    with open(config.data_file, "wb") as handle :
        pickle.dump((
            contexts, train_qa, valid_qa, context_hash
            ), handle)
    print(contexts[0])
    return contexts, train_qa, valid_qa
