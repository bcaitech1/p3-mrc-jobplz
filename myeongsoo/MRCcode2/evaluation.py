...
def main():
    ...
    if data_args.eval_retrieval:
        datasets = run_sparse_retrieval(datasets, training_args, topk = 5)

    # eval or predict mrc model
    if training_args.do_eval or training_args.do_predict:
        run_mrc(data_args, training_args, model_args, datasets, tokenizer, model, topk = 5)


def run_sparse_retrieval(datasets, training_args, topk = 5):
    #### retreival process ####
    ...
    if training_args.do_predict: # test data 에 대해선 정답이 없으므로 id question context 로만 데이터셋이 구성됩니다.
        f = Features({'context': Sequence(feature=Value(dtype='string', id=None), length=-1, id=None),
                      'id': Value(dtype='string', id=None), # question에 대응되는 id
...
def run_mrc(data_args, training_args, model_args, datasets, tokenizer, model, topk=5):
    ...
    doc_score_column_name = "scores" if "scores" in column_names else column_names[3]
    ...
    def logit2prob(logit):
        doc_score = np.array(logit)
        exp_score = np.exp((doc_score - np.max(doc_score))) # 2 for softmax smoothing need to apply as a variable! 
        probs = exp_score / exp_score.sum()
        return probs
    ...
    def prepare_validation_features(examples):
        ...
        # treat the each passage individual
        questions =[q for q in examples[question_column_name] for _ in range(topk)]
        contexts = [ctx for ctxs in examples[context_column_name] for ctx in ctxs ]
        scores = [sc for scs in examples[doc_score_column_name] for sc in logit2prob(scs)]
        ...
        for i in range(len(tokenized_examples["input_ids"])):
            ...
            sample_index = sample_mapping[i] // topk
            sample_rank = sample_mapping[i] % topk
            tokenized_examples["example_id"].append(examples["id"][sample_index])
            tokenized_examples["scores"].append(examples["scores"][sample_index][sample_rank])
            tokenized_examples["ctx_rank"].append(sample_rank)

            # Set to None the offset_mapping that are not part of the context so it's easy to determine if a token
            # position is part of the context or not.
            tokenized_examples["offset_mapping"][i] = [
                (o if sequence_ids[k] == context_index else None)
                for k, o in enumerate(tokenized_examples["offset_mapping"][i]) # 이 문제인 상황!
            ]
