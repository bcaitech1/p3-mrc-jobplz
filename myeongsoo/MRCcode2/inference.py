...
def main():
    ...
    if data_args.eval_retrieval:
        ...
        datasets = mix_dataset(dense = datasets_d, sparse = datasets, K = 0.8, cut = 100)
        datasets = dataset_formatting(datasets, training_args)

    # eval or predict mrc model
    if training_args.do_eval or training_args.do_predict:
        run_mrc(data_args, training_args, model_args, datasets, tokenizer, model, topk=100)

def mix_dataset(dense, sparse, K = 0.8, cut = 30):
    CONTEXT = []
    CONTEXT_ID = []
    SCORES = []
