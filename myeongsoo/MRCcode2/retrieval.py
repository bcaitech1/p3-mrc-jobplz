...
class SparseRetrieval:
    def __init__(self, tokenize_fn, vec_type='tfidf', data_path="/opt/ml/input/data/data/", context_path="wikipedia_documents.json"):
        ...
        self.contexts = wiki # list(dict.fromkeys([v['text'] for v in wiki.values()])) # set 은 매번 순서가 바뀌므로
        ...
        self.pickle_name = f"sparse_embedding.bin"
        self.vectorizer_name = f"tfidv.bin"
    
    def tfidf2bm25(self,tfidf_path='/opt/ml/input/data/data/sparse_embedding_tfidf.bin'):
    ...
    def get_sparse_embedding(self):
        ...
        emd_path = os.path.join(self.data_path, self.pickle_name)
        vectorizer_path = os.path.join(self.data_path, self.vectorizer_name)
        if os.path.isfile(emd_path) and os.path.isfile(vectorizer_path):
            ...
            elif self.vec_type == 'bm25' :
                tfidf_path = os.path.join(self.data_path, 'sparse_embedding_tfidf.bin')
                if os.path.isfile(tfidf_path):
                    self.p_embedding = self.tfidf2bm25(tfidf_path)
                else :
                    assert (), "sparse_embedding_tfidv.bin을 먼저 생성해주세요"
        ...
        if isinstance(query_or_dataset, str):
            ...
            for idx, example in enumerate(tqdm(query_or_dataset, desc="Sparse retrieval: ")):
                ...
                tmp = {
                    ...
                    "id": example['doc_id'] if 'doc_id' in example else example['id'], 
