...
class SparseRetrieval:
    ...
    # 위키피디아 kss로 잘라서
        ...
        if os.path.isfile(os.path.join(self.data_path, wiki1024_name)) :
            with open(os.path.join(self.data_path, wiki1024_name), 'rb') as file :
            ...
            for current in new_context :
                # print(current)
                try :
                    te = kss.split_chunks(current, max_length= 1024, overlap = True)
                    for v in te :
                    ...
                    continue
            # 위키피디아 피클 저장
            with open(os.path.join(data_path, wiki_1024_name), "wb") as file:
