...
from utils_qa_elastic import postprocess_qa_predictions, check_no_error, tokenize
...
def elastic_search(topk, post_type) :
    ...
    if not post_type :
        ...
        for question in tqdm(dataset_question) :
            ...
            docs = es.search(index='document',body=query,size=topk)['hits']['hits']
    ...
    # score
        ...
        for i, question in tqdm(enumerate(dataset_question)) :
            ...
            docs = es.search(index='document',body=query,size=topk)['hits']['hits']
            for doc in docs :
                new_dataset_id.append(dataset_id[i])    # id
                new_dataset_question.append(question)   # quesetion
                dataset_context.append(doc['_source']['text'])  # retrieve context
                new_dataset_score.append(doc['_score']) # score

        df = pd.DataFrame({'id' : new_dataset_id, 'question' : new_dataset_question, 'context' : dataset_context, 'score' : new_dataset_score})
