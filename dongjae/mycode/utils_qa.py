...
def postprocess_qa_predictions(
    ...
    for example_index, example in enumerate(tqdm(examples)):
        ...
        # 기본
            all_predictions[example["id"]] = delete_josa(predictions[0]["text"].strip())
        # 포로로꺼
            # all_predictions[example["id"]] = postprocess_span(tagger, predictions[0]["text"].strip())
        else:
            ...
            # 기본
                all_predictions[example["id"]] = delete_josa(best_non_null_pred["text"]).strip()
            # 포로로꺼
                # all_predictions[example["id"]] = postprocess_span(tagger, best_non_null_pred["text"].strip())
...
#         all_predictions[k] = delete_josa(v).strip()
...
class BM25(object):
    def __init__(self, tokenizer=tokenize, ngram_range=(1, 2), 
                        b=0.75, k1=1.6):
