...
stopword_pickle = 'stopword.pickle'

with open(os.path.join(data_path, stopword_pickle), 'rb') as file :
    stopwords = pickle.load(file)

_stopwords = ['_'+word for word in stopwords]
stopwords = stopwords + _stopwords

mecab = Mecab()

p_t = Pororo(task='tokenize', lang='ko', model = "mecab.bpe64k.ko")
...
def elastic_search(topk, post_type) :
    ...
    if not post_type :
        ...
        for question in tqdm(dataset_question) :
            ...
            for v in docs :
                temp_result = []
                temp_doc = v['_source']['text']
                tokenized_temp_doc = p_t(temp_doc)
                
                pure_doc = ''.join([word for word in tokenized_temp_doc if not word in stopwords])
                pp = pure_doc.replace('▁', ' ')
