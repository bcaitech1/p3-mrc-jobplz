...
def postprocess_qa_predictions(
    ...
    all_predictions = [collections.OrderedDict() for _ in range(4)]
    all_nbest_json = [collections.OrderedDict() for _ in range(4)]
    if version_2_with_negative:
        scores_diff_json = [collections.OrderedDict() for _ in range(4)]
    ...
    for example_index, example in enumerate(tqdm(examples)):
                    ...
                    prelim_predictions.append(
                        {
                            ...
                            "doc_s_logit_mean" : np.mean(start_logits),
                            "doc_e_logit_mean" : np.mean(end_logits),
        ...
        def softmax(x):
            ...
            e_x = np.exp(x)
            return e_x / e_x.sum()
        n_scores = softmax(np.array([pred.pop("score") for pred in prelim_predictions]))
        n_starts = softmax(np.array([pred.pop("start_logit") for pred in prelim_predictions]))
        n_ends = softmax(np.array([pred.pop("end_logit") for pred in prelim_predictions]))
        n_doc_scores = np.array([pred.pop("doc_score") for pred in prelim_predictions])
        prrobs = [n_starts * n_ends * n_doc_scores * 100, n_starts * n_ends * 100, n_starts + n_ends, (n_starts + n_ends) * n_doc_scores]
        PRELIM = prelim_predictions
        for _, probs in enumerate(prrobs) :
            prelim_predictions = copy.deepcopy(PRELIM)
            ...
            if not version_2_with_negative:
                all_predictions[_][example["id"]] = delete_josa(predictions[0]["text"])
            else:
                ...
                scores_diff_json[_][example["id"]] = float(
                    score_diff
                )  # To be JSON-serializable.
                if score_diff > null_score_diff_threshold:
                    all_predictions[_][example["id"]] = ""
                else:
                    all_predictions[_][example["id"]] = delete_josa(best_non_null_pred["text"]).strip()

            # Make `predictions` JSON-serializable by casting np.float back to float.
            all_nbest_json[_][example["id"]] = [
                {
                    k: (
                        float(v)
    ...
    for _ in range(4):
        ...
        if output_dir is not None:
            ...
            with open(prediction_file, "w") as writer:
                writer.write(json.dumps(all_predictions[_], indent=4, ensure_ascii=False) + "\n")
            logger.info(f"Saving nbest_preds to {nbest_file}.")
            with open(nbest_file, "w") as writer:
                writer.write(json.dumps(all_nbest_json[_], indent=4, ensure_ascii=False) + "\n")
            if version_2_with_negative:
                logger.info(f"Saving null_odds to {null_odds_file}.")
                with open(null_odds_file, "w") as writer:
                    writer.write(json.dumps(scores_diff_json[_], indent=4, ensure_ascii=False) + "\n")

    return all_predictions[_]
